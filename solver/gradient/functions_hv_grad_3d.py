...
def determine_mo_sol_on_ref_boundary(mo_obj_val, ref_point):
    ...
    ref_point_temp = ref_point[:, None]  # add axis so that comparison works
    boundary_booleans = np.logical_and(np.all(mo_obj_val <= ref_point_temp, axis=0),
                                       np.any(mo_obj_val == ref_point_temp, axis=0))
...
def compute_domination_properties(mo_obj_val):
    ...
    for i in range(0, n_mo_sol):
        cur_col = mo_obj_val[:, i][:, None]
        weak_domination_matrix[i, :] = np.logical_and(np.all(cur_col <= mo_obj_val, axis=0),
                                                      np.any(cur_col < mo_obj_val, axis=0))
    ...
    is_weakly_but_not_strongly_dominated = np.logical_and(np.logical_not(is_strongly_dominated),
                                                          np.any(weak_domination_matrix, axis=0))
    ...
    subset_of_coordinate_sharing_matrix_with_diag_zero = subset_of_coordinate_sharing_matrix_with_diag_zero[:,
                                                         is_not_weakly_dominated]
    ...
    is_not_weakly_dominated_and_shares_coordinates_with_other_not_weakly_dominated = np.logical_and(
        is_not_weakly_dominated, has_shared_coordinates)

    ## is not weakly dominated and share no coordinate with another not weakly dominated mo_sol
    is_not_weakly_dominated_and_shares_no_coordinates_with_other_not_weakly_dominated = np.logical_and(
        is_not_weakly_dominated, np.logical_not(has_shared_coordinates))

    return (is_strongly_dominated, is_weakly_dominated, is_weakly_but_not_strongly_dominated, is_not_weakly_dominated,
            is_not_weakly_dominated_and_shares_coordinates_with_other_not_weakly_dominated,
            is_not_weakly_dominated_and_shares_no_coordinates_with_other_not_weakly_dominated)
...
def compute_subsets(mo_obj_val, ref_point):
    is_strongly_dominated, is_weakly_dominated, is_weakly_but_not_strongly_dominated, \
        ...
        is_not_weakly_dominated_and_shares_no_coordinates_with_other_not_weakly_dominated = compute_domination_properties(
        mo_obj_val)
...
def grad_multi_sweep(mo_obj_val, ref_point):
    ...
    for k in range(0, n_obj):
        ...
        queue_index = len(
            P)  # this initialization is actually index of last queue entry +1. The +1 is convenient because the while loop will always update the index by -1, so it all matches up in the end
        T = list()
        while (len(Q) > 0):
